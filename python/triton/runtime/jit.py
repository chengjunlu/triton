...
class JITFunction(KernelInterface[T]):
    ...
    def run(self, *args, grid, warmup, **kwargs):
        ...
        def conclude_device_type(*args, **kwargs) -> str:
            ...
            device_types = map(lambda v: v.device.type
                               if hasattr(v, "device") and hasattr(v.device, "type") else "", input_values)
            pinned_memory_flags = map(
                lambda v: v.is_pinned() if hasattr(v, "is_pinned") and callable(v.is_pinned) else False, input_values)
